 ...
 */
public class LeaderboardAddSignCommand extends BaseCommand {

    @Command(name = "leaderboard.addsign", permission = "carbyne.command.leaderboard", inGameOnly = true)
    public void onCommand(CommandArgs command) {
        ...
        if (leaderboard.getSignLocations().contains(block.getLocation())) {
            MessageManager.sendMessage(player, "&cThis leaderboard already contains this sign.");
            return;
        }

        leaderboard.getSignLocations().add(block.getLocation());
        leaderboard.save();

        MessageManager.sendMessage(player, "&aSuccessfully added a sign to the leaderboard.");
    }
}
