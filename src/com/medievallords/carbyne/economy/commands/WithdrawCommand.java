...
public class WithdrawCommand extends BaseCommand {
    ...
    public void execute(CommandArgs commandArgs) {
        ...
        if (args[0].equalsIgnoreCase("all")) {
            amount = balance * getEconomyManager().getGoldWorth();
        } else {
            try {
                amount = Integer.parseInt(args[0]) * getEconomyManager().getGoldWorth();
            } catch (Exception e) {
        ...
        Map<Integer, ItemStack> leftover = player.getInventory().addItem(new ItemStack(Material.GOLD_NUGGET, amount / getEconomyManager().getGoldWorth()));
        ...
        getEconomyManager().withdraw(player.getUniqueId(), amount);
        MessageManager.sendMessage(player, "&7You have withdrawn &c" + amount / getEconomyManager().getGoldWorth() + " &7gold nuggets in exchange for &c" + (amount >= balance ? "all" : "\u00A9" + amount) + " &7credits.");
    }
}