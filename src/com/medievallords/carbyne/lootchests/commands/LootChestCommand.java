            ...
            if (args[0].equalsIgnoreCase("create")) {
                if (args.length == 5) {
                    try {
                        ...
                        getLootChestManager().getLootChests().add(new LootChest(getLootChestManager(), uniqueName, lootTableName, location, respawnString, maxItems, blockFace, health));
                        ...
                        main.getLootChestFileConfiguration().set(cp + ".Health", health);
                    ...
                    try {
                        ...
                        String mob = args[5];
                        String[] mobs = mob.split(",");

                        LootChest lc = new LootChest(getLootChestManager(), uniqueName, lootTableName, location, respawnString, maxItems, blockFace, health);
                        lc.getMobs().addAll(Arrays.asList(mobs));

                        getLootChestManager().getLootChests().add(lc);
                        ...
                        main.getLootChestFileConfiguration().set(cp + ".Face", blockFace.name());
                        main.getLootChestFileConfiguration().set(cp + ".Health", health);
                        main.getLootChestFileConfiguration().set(cp + ".Mobs", Arrays.asList(mobs));
                        main.getLootChestFileConfiguration().save(main.getLootChestFile());
                        MessageManager.sendMessage(sender, "&2Created Loot Chest!");
                    } catch (Exception e) {
                        MessageManager.sendMessage(sender, "&cFailed to create loot chest!");
                ...
                if (args.length == 2) {
                    try {
                        ...
                        if (getLootChestManager().getByLocation(location) != null) {
                            LootChest lootChest = main.getLootChestManager().getByLocation(location);
